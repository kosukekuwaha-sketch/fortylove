...
export function ChatbotPreview({ mode = "preview" }: { mode?: "preview" | "admin" | "member" }) {
  ...
  return <section className="chatbot-preview" aria-labelledby="chatbot-preview-title">
    <header><span className="chatbot-icon"><Bot /></span><div><h2 id="chatbot-preview-title">{mode === "preview" ? "動作確認チャット" : "Fortylove チャットBot"}</h2><p><ShieldCheck />{mode === "preview" ? "super_adminは常時テスト可能" : mode === "admin" ? "管理者向け" : "メンバー向け"}</p></div></header>
