...
export function AdminNav({ role }: { role: "admin" | "super_admin" }) {
  ...
  return <aside className="admin-nav">
    ...
    <details className="admin-mobile-menu"><CloseMenuOnNavigation /><summary><Brand /><span><Menu />メニュー</span></summary><nav>{links}<Link href="/profile"><UserRound />プロフィール編集</Link></nav><form action={logout}><ConfirmSubmitButton message="ログアウトしますか？"><LogOut />ログアウト</ConfirmSubmitButton></form></details>
  </aside>;
}
