...
import { Archive, BadgeCheck, CalendarDays, CircleHelp, LogOut, Menu, Settings, ShieldCheck, UserRound, UsersRound } from "lucide-react";
...
export function AdminNav({ role }: { role: "admin" | "super_admin" }) {
  const links = <><Link href="/admin"><UsersRound />新歓受付名簿</Link><Link href="/admin/members"><BadgeCheck />入会者リスト</Link><Link href="/admin/events"><CalendarDays />イベント</Link><Link href="/admin/faqs"><CircleHelp />FAQ管理</Link>{role === "super_admin" && <Link href="/admin/admins"><ShieldCheck />管理者一覧</Link>}{role === "super_admin" && <Link href="/admin/withdrawals"><Archive />退会者台帳</Link>}<Link href="/admin/settings"><Settings />設定</Link></>;
