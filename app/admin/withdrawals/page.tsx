...
export default async function Withdrawals({ searchParams }: { searchParams: Promise<{ deleted?: string; restored?: string; error?: string }> }) {
  ...
  return <section className="admin-page">
    ...
    {error && <div className="alert">{error === "selection" ? "完全削除する退会者を選択してください。" : error === "password" ? "仮パスワードは4文字以上で設定してください。" : error === "restore" ? "アカウントを復旧できませんでした。" : "退会者情報を削除できませんでした。"}</div>}
