...
export default async function Administrators() {
  ...
  return <section className="admin-page">
    ...
    <details className="create-panel"><summary>＋ 管理者権限を付与</summary><form action={updateUserRole} className="role-form"><label>登録者<select name="user_id" required><option value="">選択してください</option>{users?.filter(user => user.role === "member").map(user => <option key={user.id} value={user.id}>{user.name}（{user.university}）</option>)}</select></label><label>付与する権限<select name="role" defaultValue="admin"><option value="admin">管理者</option><option value="super_admin">最高情報責任者</option></select></label><ConfirmSubmitButton className="primary" message="選択したユーザーへ管理権限を付与しますか？">権限を付与</ConfirmSubmitButton></form></details>
