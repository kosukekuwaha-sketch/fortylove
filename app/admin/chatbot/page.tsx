import { Bot, Database, FileText, Mail, Plus, Sparkles, Upload } from "lucide-react";
import { redirect } from "next/navigation";
import { createChatbotKnowledge, deleteChatbotKnowledge, importChatbotMarkdown, updateChatbotEscalationEmail, updateChatbotKnowledge } from "@/app/chatbot-actions";
...
export default async function ChatbotAdmin({ searchParams }: { searchParams: Promise<{ created?: string; updated?: string; deleted?: string; imported?: string; email_updated?: string; error?: string }> }) {
  ...
  const [{ data: knowledge, error: knowledgeError }, { count: upcomingEvents }, { data: settings }] = await Promise.all([
    ...
    client.from("app_settings").select("chatbot_escalation_email").eq("id", 1).maybeSingle(),
  ]);
  const { created, updated, deleted, imported, email_updated: emailUpdated, error } = await searchParams;
  const completed = emailUpdated ? "有人対応の通知先メールアドレスを保存しました。" : imported !== undefined ? `${imported}件の回答データをMarkdownから下書きとして取り込みました。` : created ? "回答データを追加しました。" : updated ? "回答データを更新しました。" : deleted ? "回答データを削除しました。" : "";
  ...
  return <section className="admin-page chatbot-admin-page">
    ...
    {error && <div className="alert">{error === "validation" ? "入力内容を確認してください。キーワードは1〜20件必要です。" : error === "email-validation" ? "正しいメールアドレスを入力してください。" : error === "email-save" ? "通知先を保存できませんでした。追加マイグレーションをご確認ください。" : error === "markdown-file" ? "512KB以下のMarkdown（.md）ファイルを選択してください。" : error === "markdown-empty" ? "回答データとして取り込める文章がありませんでした。" : error === "markdown-import" ? "Markdownを取り込めませんでした。追加マイグレーションをご確認ください。" : "回答データを保存できませんでした。Supabaseのマイグレーションをご確認ください。"}</div>}
