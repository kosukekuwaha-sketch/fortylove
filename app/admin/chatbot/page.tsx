...
import { createChatbotKnowledge, deleteChatbotKnowledge, updateChatbotKnowledge } from "@/app/chatbot-actions";
...
export default async function ChatbotAdmin({ searchParams }: { searchParams: Promise<{ created?: string; updated?: string; deleted?: string; error?: string }> }) {
  ...
  const completed = created ? "回答データを追加しました。" : updated ? "回答データを更新しました。" : deleted ? "回答データを削除しました。" : "";
  ...
  return <section className="admin-page chatbot-admin-page">
    ...
    {error && <div className="alert">{error === "validation" ? "入力内容を確認してください。キーワードは1〜20件必要です。" : "回答データを保存できませんでした。Supabaseのマイグレーションをご確認ください。"}</div>}
    ...
    <details className="create-panel chatbot-create-panel"><summary><Plus /> 回答データを追加</summary><KnowledgeForm action={createChatbotKnowledge} /></details>
