...
import { deleteReceptionAccount, registerJoinedMember } from "@/app/actions";
import { DirectMembershipForm } from "@/components/direct-membership-form";
import { ConfirmSubmitButton } from "@/components/confirm-submit-button";
export const dynamic = "force-dynamic";
export default async function Members({ searchParams }: { searchParams: Promise<{ q?: string; university?: string; page?: string; error?: string; password_reset?: string; membership_registered?: string; deleted?: string }> }) {
  const { q = "", university = "", page = "1", error, password_reset, membership_registered, deleted } = await searchParams; const size = 20; const from = (Number(page) - 1) * size;
  ...
  return <section className="admin-page"><div className="page-title"><div><p className="eyebrow green">MEMBERS</p><h1>新入生名簿</h1><p>登録者の情報を検索・確認できます。</p></div><span className="stat"><strong>{count ?? 0}</strong>名 登録中</span></div>
    ...
    {deleted && <div className="success-message">新歓受付登録アカウントを削除しました。</div>}
    {error && <div className="alert">{error === "password" ? "仮パスワードは4文字以上で入力してください。" : error === "membership-register" ? "入会者として登録できませんでした。" : error === "delete" ? "アカウントを削除できませんでした。" : "パスワードを更新できませんでした。"}</div>}
    ...
    <div className="table-wrap"><table><thead><tr><th>氏名</th><th>大学</th><th>学部・学科</th><th>学年</th><th>テニス経験</th><th>ラケット</th><th>SNS・連絡先</th><th>登録日</th>{isSuperAdmin && <th>操作</th>}</tr></thead><tbody>{data?.map(m => <tr key={m.id}><td><span className="table-name"><i>{m.name[0]}</i>{m.name}</span></td><td>{m.university}</td><td>{m.faculty}<small>{m.department}</small></td><td>{Number(m.grade) >= 5 ? "4年以上" : `${m.grade}年`}</td><td>{m.tennis_experience || "未記入"}</td><td><span className={m.has_racket ? "status-owned" : "status-needed"}>{m.has_racket ? "所持" : "未所持"}</span></td><td><InstagramLink id={m.instagram_id} /><small>{m.line_display_name && `LINE表示名: ${m.line_display_name}`}</small></td><td>{new Date(m.created_at).toLocaleDateString("ja-JP")}</td>{isSuperAdmin && <td><form action={deleteReceptionAccount}><input type="hidden" name="user_id" value={m.id} /><ConfirmSubmitButton className="danger table-withdraw" message={`${m.name}さんのアカウントと予約情報を削除します。退会者台帳から復旧できます。実行しますか？`}>削除</ConfirmSubmitButton></form></td>}</tr>)}</tbody></table>{!data?.length && <div className="empty"><UserPlus /><p>該当するメンバーはいません</p></div>}</div>
  </section>;
}
