...
export async function resetUserPassword(fd: FormData) {
  ...
  if (!userId || !isValidNewPassword(temporaryPassword)) redirect("/admin?error=password");
  const client = db();
...
export async function restoreWithdrawalAccount(fd: FormData) {
  ...
  const temporaryPassword = text(fd, "temporary_password");
  if (!isValidNewPassword(temporaryPassword)) redirect("/admin/withdrawals?error=password");
  const client = db();
